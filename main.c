...
int main() {
  ...
  char name_file_first[50];
  char name_file_second[50];

  error_processing(
      read(STDIN_FILENO, name_file_first, sizeof(name_file_first)) == -1,
      "error with reading file 1");
  error_processing(
      read(STDIN_FILENO, name_file_second, sizeof(name_file_second)) == -1,
  ...
  while (read(STDIN_FILENO, line, 10) > 5) {
    if (strcmp(line, "!\n") == 0) {
      break;
    }
    if (rand() % 100 < 80) {
      ...
      write(pipe1[2], line, strlen(line));
    }
  }
